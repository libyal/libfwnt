...
LIBRARY_TESTS="access_control_entry access_control_list error lznt1 lzxpress security_descriptor security_identifier support";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fwnt_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfwnt" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
